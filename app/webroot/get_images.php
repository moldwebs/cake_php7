...
if(strpos(basename($img), '.') !== false) if(strpos($img, 'http://') === false && !is_file($img)){
    //http://clic.md/sys_files/0/small/1351188501_1.jpg
    ///var/www/webs/data/www/wrk.webs.md/webcons/prj_eldorado/www/web_views/attachments/uploads/0/images/thumb/1351188501_1.jpg
    if(substr_count($img, '/large/') > 0){
        //$img = 'http://clic.md/sys_files/0/large/' . basename($img);
    } else {
        //$img = 'http://clic.md/sys_files/0/small/' . basename($img);
    }
    //exit($img);
    //$img = str_replace('prj_mamutshop', 'prj_turboshop', $img);
}

if(strpos($_SERVER['HTTP_HOST'], 'wrk.') !== false && !is_file($img) && 1==2){
    $_image = ltrim(basename($image), '0_');
    if(substr_count($image, '/large/') > 0){
        $img = "http://clic.md/sys_files/0/large/{$_image}";
    } else {
        $img = "http://clic.md/sys_files/0/small/{$_image}";
    }
}
