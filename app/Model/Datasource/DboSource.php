 ...
 */
	protected function _execute($sql, $params = array(), $prepareOptions = array()) {
	    //if(preg_match('/(update |delete |insert )/i', strtolower($sql)) && !preg_match('/(wb_cms_sessions|wb_cms_stats)/i', strtolower($sql))) file_put_contents(LOGS . DS . 'db_ops_'.date("Ymd").'.log', date("Y-m-d H:i:s")."-{$sql}-{$_SERVER['REQUEST_URI']}-\r\n" . "\r\n", FILE_APPEND);
		$sql = trim($sql);
 ...
 */
	public function fetchAll($sql, $params = array(), $options = array()) {
        Configure::write('TMP.lquery', $sql);
        ...
        //if(Configure::read('TMP.sql_no_cache') != '1' && strpos($sql, 'RANDOFF()') === false){
        if(Configure::read('TMP.sql_no_cache') != '1'){
            //if(strpos($sql, 'RAND()') === false && strpos($sql, 'Session') === false && strpos($sql, 'WsActivity') === false){
            //echo 'OK'.$sql . "<br>";
            if($results = Cache::read(md5($sql), (strpos($sql, 'RAND()') === false ? 'query' : 'query_1'))) return ($results == 'EMPTY' ? array() : $results);
        } else {
            //echo 'KO'.$sql . "<br>";
        }
		if ($result = $this->execute($sql, array(), $params)) {
            ...
            if(Configure::read('TMP.sql_no_cache') != '1') Cache::write(md5($sql), (empty($out) ? 'EMPTY' : $out), (strpos($sql, 'RAND()') === false ? 'query' : 'query_1'));
	...
	public function fetchVirtualField(&$result) {
		if (isset($result[0]) && is_array($result[0])) {
			foreach ($result[0] as $field => $value) {
				...
				if ($model->isVirtualField($virtual) || 1 == 1) {
 ...
 */
	public function logQuery($sql, $params = array()) {
        ...
        if($this->took >= 5000){
            file_put_contents(LOGS . DS . md5($sql) . '.qlog', "TIME:{$this->took}\r\nQUERY:{$sql}");
        }
	...
	public function read(Model $model, $queryData = array(), $recursive = null) {
		...
		foreach ($_associations as $type) {
			foreach ($model->{$type} as $assoc => $assocData) {
                // MYX
                if ($model->recursive == -1 && empty($assocData['force'])) continue;
        ...
        // MYX 
        if($queryData['fields'][0] == 'COUNT(*) AS `count`'){
            //foreach($queryData['joins'] as $key => $val) if($val['type'] == 'LEFT') unset($queryData['joins'][$key]);
        }
		
        $query = trim($this->generateAssociationQuery($model, null, null, null, null, $queryData, false, $null));
        //if($_results = Cache::read(md5('read'.$query), 'query')) return $_results;
		...
		if ($queryData['callbacks'] === true || $queryData['callbacks'] === 'after') {
            ...
            if((microtime(true) - $_microtime_start) > 0.1) Configure::write('EXEC_TIME_LOGS_read', am(Configure::read('EXEC_TIME_LOGS_read'), array('fltr-'.round(microtime(true) - $_microtime_start, 4).'s')));
		}
        
        
        $force_assoc = false;
        if(!($model->recursive > -1) && !empty($model->hasMany['I18n'])){
            $_associations = array('hasMany');
            $force_assoc = array('I18n', 'CmsAlias');
        }

		if ($model->recursive > -1 || $force_assoc) {
			$joined = array();
			if (isset($queryData['joins'][0]['alias'])) {
				$joined[$model->alias] = (array)Hash::extract($queryData['joins'], '{n}.alias');
			}
            
            $__microtime_start = microtime(true);
            
			foreach ($_associations as $type) {
				foreach ($model->{$type} as $assoc => $assocData) {
				    
                    
				    if($force_assoc) if(!in_array($assoc, $force_assoc)) continue;
					...
					if (isset($db) && method_exists($db, 'queryAssociation')) {
                        ...
                        if((microtime(true) - $_microtime_start) > 0.1) Configure::write('EXEC_TIME_LOGS_read', am(Configure::read('EXEC_TIME_LOGS_read'), array("fltr-{$assoc}".round(microtime(true) - $_microtime_start, 4).'s')));
						unset($db);
            ...
            if((microtime(true) - $__microtime_start) > 0.1) Configure::write('EXEC_TIME_LOGS_read', am(Configure::read('EXEC_TIME_LOGS_read'), array("fltr-assoc".round(microtime(true) - $__microtime_start, 4).'s')));
            
			if ($queryData['callbacks'] === true || $queryData['callbacks'] === 'after') {
				$_microtime_start = microtime(true);
                $this->_filterResults($resultSet, $model, $filtered);
                if((microtime(true) - $_microtime_start) > 0.1) Configure::write('EXEC_TIME_LOGS_read', am(Configure::read('EXEC_TIME_LOGS_read'), array('call-'.round(microtime(true) - $_microtime_start, 4).'s')));
			}
		}
        ...
        if((microtime(true) - $microtime_start) > 0.1){
            Configure::write('EXEC_TIME_LOGS_read', am(Configure::read('EXEC_TIME_LOGS_read'), array((microtime(true) - $microtime_start) . 's / ' . $query)));
        }
        if((microtime(true) - $microtime_start) > 0.1) Configure::write('EXEC_TIME_LOGS_read', am(Configure::read('EXEC_TIME_LOGS_read'), array('READ-' . $model->alias . '-'. round(microtime(true) - $microtime_start, 4))));
        
        //if(!empty($resultSet)) Cache::write(md5('read'.$query), $resultSet, 'query');
 ...
 */
	protected function _filterResults(&$results, Model $model, $filtered = array()) {
		...
		foreach ($keys as $className) {
            ...
            // MYX MODIFY
            $return = $linkedModel->Behaviors->trigger(
                'afterFind',
                array(&$linkedModel, $results, false),
			...
			foreach ($results as &$result) {
                if(empty($result[$className])) continue;
                $data = array(array($className => $result[$className]));
                $data = $linkedModel->afterFind($data, false);
					...
					if ($q !== false) {
						...
						if (($type === 'belongsTo' || $type === 'hasOne') && isset($row[$linkModel->alias], $joined[$model->alias]) && in_array($linkModel->alias, $joined[$model->alias])) {
							...
							if (!empty($joinedData)) {
						      ...
						      if(strpos($q, '`ModCurrency`.`currency` = `ObjOptPrice`.`currency`') === false){
						          $fetch = $this->fetchAll($q, $model->cacheQueries);
						      }
						}
					}
				...
				if (!empty($fetch) && is_array($fetch)) {
					if ($recursive > 0) {
						foreach ($linkModel->associations() as $type1) {
							foreach ($linkModel->{$type1} as $assoc1 => $assocData1) {
                                // MYX
                                if($assocData1['recursive'] < 2) continue;
								$deepModel = $linkModel->{$assoc1};
 ...
 */
	public function generateAssociationQuery(Model $model, $linkModel, $type, $association, $assocData, &$queryData, $external, &$resultSet) {
		...
		if (isset($query)) {
            // MYX MODIFY
            $query = $linkModel->Behaviors->trigger(
                'beforeFind',
                array(&$linkModel, $query),
                array('modParams' => true)
            );
            // MYX MODIFY
