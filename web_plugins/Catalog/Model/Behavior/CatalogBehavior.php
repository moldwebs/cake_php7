...
class CatalogBehavior extends ModelBehavior {
    ...
    public function beforeFind(Model $model, $query) {
        ...
        if(!$model->Behaviors->enabled('Currency')) $model->virtualFields['price_conv'] = "`ObjItemList`.`price`";
    ...
    public function afterFind(Model $model, $results, $primary) {
        ...
        Configure::write('EXEC_TIME_LOGS', am(Configure::read('EXEC_TIME_LOGS'), array('CatalogBehavior1:' . date("Y-m-d H:i:s"))));
        ...
        if(!empty($results)) foreach($results as $key => $result){
            ...
            if(Configure::read('CMS.catalog_title_category') == '1' && !empty($result['ObjItemTree']['title_prod'])){
                $results[$key][$model->alias]['title'] = $result['ObjItemTree']['title_prod'] . ' ' . $results[$key][$model->alias]['title'];
            }
        }
        ...
        if(!empty($results) && Configure::read('is_admin') != '1' || Configure::read('TMP.force_combinations') == '1'){
            ...
            if($result[$model->alias]['tid'] == 'catalog'){
                ...
                foreach($results as $key => $result){
                    ...
                    if($result[$model->alias]['status'] == '0'){
                        $results[$key]['Price']['value'] = 0;
                        $results[$key][$model->alias]['price_conv'] = 0;
                        $results[$key][$model->alias]['price'] = 0;
                        $results[$key]['Price']['html_value'] = Configure::read('CMS.settings.'.$result[$model->alias]['tid'].'.no_price_txt');
                        $results[$key]['Price']['html_currency'] = '';
                        //pr($results[$key][$model->alias]['title']);
                    } else {
                        //pr($results[$key][$model->alias]['title']);
                    }
                }
        ...
        Configure::write('EXEC_TIME_LOGS', am(Configure::read('EXEC_TIME_LOGS'), array('CatalogBehavior2:' . date("Y-m-d H:i:s"))));
